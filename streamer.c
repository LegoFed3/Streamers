...
extern double fixed_playout_delay;
extern int opmode;
extern int pullmode;
...
static void print_usage(int argc, char *argv[])
{
  fprintf (stderr,
    ...
    "\t[--fixed_playout_delay]: set a fixed (positive) playout delay (in seconds)."
    "Recommend to use the -o option to set the output buffer to a suitably large value as well, or the streamer will hang\n"
  ...
  static struct option long_options[] = {
    ...
    switch(o) {
      case 0: //for long options
        ...
        if( strcmp( "fixed_playout_delay", long_options[option_index].name ) == 0 ) { fixed_playout_delay = atof(optarg); }
